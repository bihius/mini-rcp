...
def archive_file(events_file, archive_folder):
    ...
    # Check if archive folder is SMB path
    is_smb_archive = archive_folder.startswith('\\\\') or archive_folder.startswith('//')
    ...
    if events_file.startswith('\\\\') or events_file.startswith('//'):
        ...
        # For SMB files, we need to read the content and write to archive
        try:
            ...
            if is_smb_archive:
                # For SMB archive, use timestamp-based naming to avoid directory listing
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                dest_filename = f"{base_name}.{timestamp}"
                dest_path = f"{archive_folder.rstrip('/')}/{dest_filename}".replace('/', '\\')

                # Write content to SMB archive using UNC path
                try:
                    with open(dest_path, 'w', encoding='utf-8') as f:
                        f.write(content)
                    logger.info(f"Archived SMB file {events_file} to SMB archive {dest_path}")
                    return
                except Exception as smb_write_e:
                    logger.error(f"Failed to write to SMB archive: {smb_write_e}")
                    raise
            else:
    ...
    if is_smb_archive:
        # For SMB archive, use timestamp-based naming and direct UNC write
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        dest_filename = f"{base_name}.{timestamp}"
        dest_path = f"{archive_folder.rstrip('/')}/{dest_filename}".replace('/', '\\')

        try:
            shutil.move(events_file, dest_path)
            logger.info(f"Archived {events_file} to SMB archive {dest_path}")
            return
        except Exception as smb_move_e:
            logger.error(f"Failed to move to SMB archive: {smb_move_e}")
            raise
    else:
        # Local archive directory
        archive_dir = archive_folder
